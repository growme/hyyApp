...
public class SbCommonProblemDao extends BaseDao<SbCommonProblem> {
    ...
    public List<SbCommonProblem> findProblemList(SbCommonProblem sbCommonProblem) {
        ...
        if (CPSUtil.isNotEmpty(sbCommonProblem.getTitle())) {
            this.appendWhere(sql);
            sql.append(" and title like ? ");
            params.add("%"+sbCommonProblem.getTitle()+"%");
        }
        if (CPSUtil.isNotEmpty(sbCommonProblem.getStatus())) {
            this.appendWhere(sql);
            sql.append(" and status = ? ");
            params.add(sbCommonProblem.getStatus());
        }
        if (CPSUtil.isNotEmpty(sbCommonProblem.getType())) {
            this.appendWhere(sql);
            sql.append(" and type = ? ");
            params.add(sbCommonProblem.getType());
        }

        sql.append(" order by type asc,sort asc,create_time desc");
        List<SbCommonProblem> list = memory.query(sql, new BeanListHandler<SbCommonProblem>(SbCommonProblem.class), params);
        return  list;
    }
}
